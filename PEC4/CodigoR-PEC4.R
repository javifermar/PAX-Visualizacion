...
data_gather <- data_def[,c(6, 13:28)]

data_grupos <- gather(data = data_gather, key="Reference", value = "Yes/No", 2:17)
data_grupos <- data_grupos[which(data_grupos$`Yes/No`=='Yes'),c(1,2)]
data_grupos_unique <- unique(data_grupos)
remove(data_grupos)
remove(data_gather)

write_xlsx(x = list(references=data_grupos_unique), path = './references.xlsx', col_names = TRUE)
...
columnas <- c('Ref.Children/Youth',
              'Ref.Disabled persons',
              "Ref.Older persons",
              "Ref.Migrant workers",
              "Ref.Racial/Ethnic",
              "Ref.Religious groups",
              "Ref.Indigenous people",
              "Ref.Refugees/ displaced persons",
              "Ref.Social Class",
              "Ref.Women rights",
              "Ref.LGBTI persons",
              "Ref.Families",
...
grupos <- rbind(table(my_dataset[,columnas[1]]), 
                table(my_dataset[,columnas[2]]),
                table(my_dataset[,columnas[3]]))

grupos <- NULL
for (i in 1:15) {
  grupos <- rbind(grupos,
                  table(my_dataset[,columnas[i]]))
}

rownames(grupos) <- columnas
